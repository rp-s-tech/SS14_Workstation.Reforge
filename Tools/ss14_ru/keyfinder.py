...
class FilesFinder:
    ...
    def get_relative_path_dict(self, file: FluentFile, locale):
        if locale == 'ru-RU':
            return RelativeFile(file=file, locale=locale,
                                relative_path_from_locale=file.get_relative_path(self.project.ru_primary_layer_dir_path))
        elif locale == 'en-US':
            return RelativeFile(file=file, locale=locale,
    ...
    def get_file_pair(self, en_file: FluentFile) -> typing.Tuple[FluentFile, FluentFile]:
        ru_file_path = self.project.get_ru_primary_fluent_file_path(en_file.full_path)
    ...
    def get_files_pars(self):
        ...
        ru_fluent_files = self.project.get_fluent_files_by_dir(project.ru_primary_layer_dir_path)
    ...
    def create_ru_analog(self, en_relative_file: RelativeFile) -> FluentFile:
        ...
        if not os.path.exists(ru_file_path):
            ru_file.save_data('')
            logging.info(f'Создан файл слоя rpsx: {ru_file_path}')
    ...
    def warn_en_analog_not_exist(self, ru_relative_file: RelativeFile):
        ...
        en_file_path = os.path.join(
            ...
            os.path.relpath(file.full_path, self.project.ru_primary_layer_dir_path)
        )
...
class KeyFinder:
    def __init__(self, files_dict):
        ...
        self.duplicates_with_base: typing.List[typing.Tuple[str, str, str]] = []
        self.other_ru_key_index = {}

    def execute(self) -> typing.List[FluentFile]:
        self.changed_files = []
        self.duplicates_with_base = []
        self.other_ru_key_index = self.build_other_ru_key_index()
        ...
        if len(self.duplicates_with_base):
            unique_duplicates = sorted(set(self.duplicates_with_base))
            logging.warning('Найдены дубли ключей между ru-RU/rpsx и другими слоями ru-RU:')
            for key, rpsx_path, base_path in unique_duplicates:
                logging.warning(f'  key="{key}" rpsx="{rpsx_path}" other="{base_path}"')
    ...
    def compare_files(self, en_file, ru_file):
        ...
        self.log_duplicates_in_rpsx_file(ru_file, ru_file_parsed)
    ...
    def write_to_ru_files(self, ru_file, ru_file_parsed, en_file_parsed):
        for idx, en_message in enumerate(en_file_parsed.body):
            ...
            en_message_id = FluentAstAbstract.get_id_name(en_message)
            if en_message_id in self.other_ru_key_index:
                for duplicate_file_path in self.other_ru_key_index[en_message_id]:
                    self.duplicates_with_base.append((en_message_id, ru_file.full_path, duplicate_file_path))
                continue
    ...
    def find_message_by_id(self, messages, message_id):
        if not message_id:
            return None

        return py_.find(messages, lambda message: FluentAstAbstract.get_id_name(message) == message_id)

    def build_other_ru_key_index(self):
        key_index = {}
        other_ru_files = project.get_ru_non_primary_fluent_files()

        for ru_file in other_ru_files:
            try:
                ru_file_parsed = ru_file.parse_data(ru_file.read_data())
            except Exception:
                continue

            for element in ru_file_parsed.body:
                ...
                if key_name not in key_index:
                    key_index[key_name] = set()

                key_index[key_name].add(ru_file.full_path)
    ...
    def log_duplicates_in_rpsx_file(self, ru_file, ru_file_parsed):
        for ru_message in ru_file_parsed.body:
            if isinstance(ru_message, ast.ResourceComment) or isinstance(ru_message, ast.GroupComment) or isinstance(ru_message, ast.Comment):
                continue

            ru_message_id = FluentAstAbstract.get_id_name(ru_message)
            if not ru_message_id:
                continue

            if ru_message_id not in self.other_ru_key_index:
                continue

            for duplicate_file_path in self.other_ru_key_index[ru_message_id]:
                self.duplicates_with_base.append((ru_message_id, ru_file.full_path, duplicate_file_path))
